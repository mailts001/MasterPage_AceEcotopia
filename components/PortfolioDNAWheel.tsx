...
const TICKER_INFO: Record<string, { sector: string; desc: string }> = {
  ...
  GE:   { sector: 'Industrials — Aerospace & Defence', desc: 'GE Aerospace — jet engines (LEAP, GE9X) for commercial and military aviation. Renamed from General Electric after the GE Vernova spinoff in 2024. Pure-play aerospace.' },
  HON:  { sector: 'Industrials — Diversified', desc: 'Honeywell — aerospace equipment, building automation, safety & productivity solutions, and industrial process controls. Defensive-growth industrial conglomerate.' },
  CAT:  { sector: 'Industrials — Heavy Machinery', desc: 'Caterpillar — world\'s largest maker of construction/mining equipment, diesel engines, and industrial turbines. Proxy for global infrastructure spending.' },
  BA:   { sector: 'Aerospace & Defence', desc: 'Boeing — commercial jets (737, 787, 777X) and US military aircraft/missiles. Recovering from production quality issues; duopoly with Airbus.' },
  LMT:  { sector: 'Aerospace & Defence', desc: 'Lockheed Martin — largest US defence contractor. F-35, Sikorsky helicopters, missile systems. High visibility through multi-year government contracts.' },
  V:    { sector: 'Payments Network', desc: 'Visa — world\'s largest card payment network by volume. Asset-light model; earns fees on transaction value not credit risk. Duopoly with Mastercard.' },
  MA:   { sector: 'Payments Network', desc: 'Mastercard — global card payments network. Near-identical business model to Visa; both benefit from secular shift from cash to digital payments.' },
  JNJ:  { sector: 'Healthcare — Diversified', desc: 'Johnson & Johnson — pharmaceuticals (immunology, oncology) and medtech (Ethicon, DePuy). Spun off consumer (Kenvue) in 2023. Defensive dividend payer.' },
  LLY:  { sector: 'Pharmaceuticals — GLP-1 / Obesity', desc: 'Eli Lilly — Mounjaro/Zepbound (GLP-1) for diabetes/obesity; blockbuster growth. Also Alzheimer\'s (donanemab) and immunology pipeline.' },
  NFLX: { sector: 'Streaming / Entertainment', desc: 'Netflix — 270M+ subscribers globally. Profitable ad-supported tier growing; live sports rights. Dominant streaming platform with strong content moat.' },
  WMT:  { sector: 'Retail — Mass Market', desc: 'Walmart — world\'s largest retailer by revenue. Growing e-commerce and advertising. Defensive; low-price positioning benefits in economic downturns.' },
  COST: { sector: 'Retail — Wholesale Club', desc: 'Costco — membership-based warehouse retail. Extremely loyal customer base; membership fees = nearly all profit. Compounding growth at scale.' },
